...
# MAGIC %pip install --upgrade mlflow pydantic
...
EVALUATION_SET_FQN = f"`{UC_CATALOG}`.`{UC_SCHEMA}`.{RAG_APP_NAME}_evaluation_set_ccv"
...
POC_DATA_PIPELINE_RUN_NAME = "data_pipeline_bcp_document_poc"
# Chain MLflow run name
POC_CHAIN_RUN_NAME = "bcp_document_poc_ccv"

# COMMAND ----------

print(f"RAG_APP_NAME {RAG_APP_NAME}")
print(f"catalog_demo_rag {UC_CATALOG}")
print(f"agent_demo_adopcion {UC_SCHEMA}")
